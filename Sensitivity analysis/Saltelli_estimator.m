...
%% Run the model to generate output from the A, B, and AB parameters
fA = zeros(N,m); %sample size by output size <-- output is the different measurement criteria
fB = zeros(N,m);
fAB = zeros(N*d,m);

% add parfor wait bar for running A,B samples
w = waitbar(0,'Start parallel running of A and B samples...');
...
parfor i=1:N
   fA(i,:) = modelfun(par_A(i,:)); % for specific parameter set, run model and get result changes
   fB(i,:) = modelfun(par_B(i,:)); 
   
   send(D,[]); %updates the wait bar (lol)
end

delete(w);
disp('Finished running the A and B samples')
% add parfor wait bar for running AB sample
w = waitbar(0,'Start parallel running of AB samples...');
D = parallel.pool.DataQueue;
afterEach(D, @parforWaitbar);
...
delete(w);
disp('Finished running the AB samples')
